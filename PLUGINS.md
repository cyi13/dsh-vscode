...
| `dsh-vscode-bridge` | `plugins/dsh-vscode-bridge` | 页面内清晰缩放 + VS Code webview 剪贴板桥 | 必需（清晰缩放、复制粘贴） |

> 两者职责不同：deeplink 解决“定位会话”，bridge 解决页面内清晰缩放和“webview 剪贴板被禁”。缺一个，扩展的部分功能即不可用。
...
```sh
cd plugins/dsh-vscode-bridge
npm install
npm run build          # 生成 lib/
npm pack               # 生成 dsh-vscode-bridge-0.1.0.tgz
